 ...
 * 
 * **** this is the main class for Oauth2
 * to get the JWT token from the request, validate it, 
 * load the user associated with the token, and pass it to Spring Security
 * 
 * We’re first parsing the JWT retrieved from the Authorization header of the request and obtaining the user’s Id. After that, We’re loading the user’s details from the database and setting the authentication inside spring security’s context.
